...
export async function callGroqLlama3(input: { prompt: string }) {
  ...
  try {
    ...
    if (!res.ok) {
      const errMsg = `Groq API error: ${res.status} ${res.statusText}`;
    ...
    console.error("Groq会話APIエラー:", err);
    throw new Error("Groq会話APIエラー: " + (err instanceof Error ? err.message : String(err)));
  }
}
